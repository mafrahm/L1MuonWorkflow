...
setup_l1m() {
    ...
    #   - "L1M_", for features provided by the analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "L1M_" variables is documented below.
    #
    ...
    if [ "${L1M_SETUP}" = "1" ]; then
        >&2 echo "the L1MuonWorkflow analysis was already succesfully setup"
    ...
    #
    # global variables
    # (L1M = L1MuonWorkflow, CF = columnflow)
    #

    # start exporting variables
    export L1M_BASE="${this_dir}"
    export CF_BASE="${this_dir}/modules/columnflow"
    export CF_REPO_BASE="${L1M_BASE}"
    ...
    if [ "${CF_REMOTE_JOB}" != "1" ]; then
        cf_setup_interactive_body() {
            ...
            query CF_DATA "Local data directory" "\$L1M_BASE/data" "./data"
            query CF_STORE_NAME "Relative path used in store paths (see next queries)" "l1m_store"
            ...
            query L1M_BUNDLE_CMSSW "Install and bundle CMSSW sandboxes for job submission?" "True"
        }
        cf_setup_interactive "${CF_SETUP_NAME}" "${L1M_BASE}/.setups/${CF_SETUP_NAME}.sh" || return "$?"
    fi
    ...
    export PATH="${L1M_BASE}/bin:${PATH}"
    export PYTHONPATH="${L1M_BASE}:${L1M_BASE}/modules/cmsdb:${PYTHONPATH}"

    # initialze submodules
    if [ -e "${L1M_BASE}/.git" ]; then
        local m
        for m in $( ls -1q "${L1M_BASE}/modules" ); do
            cf_init_submodule "${L1M_BASE}" "modules/${m}"
        done
    fi
    ...
    export LAW_HOME="${L1M_BASE}/.law"
    export LAW_CONFIG_FILE="${L1M_BASE}/law.cfg"
...
main() {
    ...
    if setup_l1m "$@"; then
        cf_color green "L1MuonWorkflow analysis successfully setup"
        return "0"
    else
        local code="$?"
